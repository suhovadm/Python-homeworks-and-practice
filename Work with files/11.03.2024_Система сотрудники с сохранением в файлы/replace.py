...
def perform_replace():
    try:
        ...
        # Открываем файл IN.txt на чтение с кодировкой utf8, задаём ему алиас f4 для дальнейших манипуляций.
        # Конструкция with обеспечивает автоматическое закрытие файла после завершения блока.
        with open('IN.txt', 'r', encoding='utf8') as f4:
    ...
    # Запись в файл.
    # Снова открываем файл IN.txt, только теперь уже на чтение, снова в кодировке utf8, опять задаём алиас f4
    # и через этот алиас делаем запись в переменную new_content.
    with open('IN.txt', 'w', encoding='utf8') as f4:
